...
def novacompo(request):
    ...
    if request.method == "POST":
        if form2.is_valid():
            ...
            composiciocreada = infocompos(autor=usuario, titol=titol, instrument=instrument, genere=genere, bpms=bpms, tonalitat=tonalitat, definicio=definicio, arxiu=arxiu, imatge_compo=imatge_compo)
            composiciocreada.save()
            usuario.composicions.add(composiciocreada)
